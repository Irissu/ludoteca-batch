...
public class JobGameCompletionNotificationListener implements JobExecutionListener {
    ...
    public void afterJob(JobExecution jobExecution) {
        if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
            ...
            String query = "SELECT title, stock FROM game";
            jdbcTemplate.query(query, (rs, row) -> new GameAvailability(rs.getString(1), rs.getString(2))).forEach(game -> LOGGER.info("Results : {}, in the database.", game));
        }
    }
}
